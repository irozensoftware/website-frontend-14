    ...
    <>
      <div className="max-w-full bg-white shadow-[0px_2px_3px_-1px_rgba(0,0,0,0.1),0px_1px_0px_0px_rgba(25,28,33,0.02),0px_0px_0px_1px_rgba(25,28,33,0.08)]  rounded-b-sm overflow-hidden">
        <div className="max-h-62.5 relative group ">
          {/* Date Badge */}
          <div className="absolute  left-3 top-3 z-50 bg-white text-center text-black-muted shadow-lg px-1.5 py-1 font-semibold">
            <p className="text-sm md:text-base">{moment(blog?.create_at).format("DD")}</p>
          ...
          <div className="relative h-full overflow-hidden">
            ...
            />
            {/* Gray overlay */}
            <div className="absolute inset-0 bg-[#00000060] bg-opacity-50 opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
          </div>
        ...
        <div className="p-4 pt-5 text-center">
          <Link href={`/blog/${blog?.slug}`} className="mt-5 block text-sm md:text-base font-medium md:font-semibold text-black-base hover:text-black-muted duration-200 cursor-pointer">
            {blog?.title}
          </Link>
