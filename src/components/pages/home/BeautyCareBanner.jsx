...
const BeautyCareBanner = () => {
  ...
  const topProdut = data?.data?.[0];
  console.log(topProdut, "ddd");
  return (
          ...
          <div className="flex flex-col md:flex-row items-center justify-between min-h-[300px] md:min-h-[350px]">
            ...
            <div className="w-full md:w-1/2 p-8 md:p-12 lg:p-16 text-center md:text-left">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center md:justify-start gap-4">
                ...
                <Link
                  href={`/product/${topProdut?.slug}`}
                  className="text-white font-semibold px-8 py-3 border-2 border-white rounded-full hover:bg-white hover:text-black transition-all duration-300 uppercase tracking-wide"
                >
                  Read More
                </Link>
              </div>
                ...
                <div className="absolute inset-0 flex items-center justify-center opacity-10">
                  <div className="grid grid-cols-3 gap-4">
                    {[...Array(9)].map((_, i) => (
                      <div
                        key={i}
                        className="w-16 h-16 border-4 border-gray-600 rounded-lg"
                      ></div>
                    ))}
                  </div>
                </div>
